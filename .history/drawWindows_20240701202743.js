...
export function updateRoomWindow(room) {
  ...
  roomWindow.innerHTML = `
    ...
    <p><strong>Exits:</strong> currentRoom.exits.forEach(exit => {logToWorld(`- ${exit.direction}: ${exit.description}`)</p>
  `;
...
export function displayCurrentRoom(player) {
    ...
    if (currentRoom.exits) {
        logToWorld('Exits:');
        currentRoom.exits.forEach(exit => {logToWorld(`- ${exit.direction}: ${exit.description}`);
        });
    }
