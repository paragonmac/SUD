...
roomWindow.innerHTML = `<h3>${room.id}</h3><p>${room.detailed_description || ''}</p><p><strong>Items:</strong> ${room.items.map(item => item.name).join(', ')}</p><p><strong>Monsters:</strong> ${room.monsters.map(monster => monster.name).join(', ')}</p>`;
