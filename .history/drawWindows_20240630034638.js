...
export function updateRoomWindow(room) {
  ...
  roomWindow.innerHTML = `
    <h3>${room.id}</h3>
    <p>${room.detailed_description || ''}<span style="color: yellow;">
        <strong>Items nearby:</strong> ${room.items.map(item => item.name).join(', ')}
    </span></p>
    <p><strong style>Monsters:</strong> <span style="color: red;"> ${room.monsters.map(monster => monster.name).join(', ')}<</p>
    <p><strong>Exits:</strong> ${room.exits.map(exit => exit.direction).join(', ')}</p>
  `;
