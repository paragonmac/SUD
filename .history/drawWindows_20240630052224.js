...
export function updateRoomWindow(room) {
  ...
  roomWindow.innerHTML = `
    <h3>${room.name}</h3>
        ...
        <strong>Items nearby:</strong> <span style="color: yellow;">${room.items.map(item => item.name).join(', ')}
    </span></p>
...
export function displayCurrentRoom() {
    const currentRoom = window.gameWorld.rooms[Player]; // Assuming the player starts in the first room
