...
export function displayCurrentRoom(player) {
    ...
    if (currentRoom.exits) {
        for (let i = 0; i < data.length; i++) {
            if (data[i].id === 1) {
            if (data[i].north !== undefined) {
                console.log('North:', data[i].north);
            }
            if (data[i].south !== undefined) {
                console.log('South:', data[i].south);
            }
            if (data[i].east !== undefined) {
                console.log('East:', data[i].east);
            }
            if (data[i].west !== undefined) {
                console.log('West:', data[i].west);
            }
            return; // Exit the loop once the target is found
            }
        }
    }
        function printDirections(data) {
  for (let i = 0; i < data.length; i++) {
    if (data[i].id === 1) {
      if (data[i].north !== undefined) {
        console.log('North:', data[i].north);
      ...
      if (data[i].south !== undefined) {
        console.log('South:', data[i].south);V
      }
      if (data[i].east !== undefined) {
        console.log('East:', data[i].east);
      }
