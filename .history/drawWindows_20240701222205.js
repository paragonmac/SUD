...
export function displayCurrentRoom(player) {
    ...
    if (currentRoom.exits) {
        if (currentRoom.exits.north) {
            logToWorld(`- ${currentRoom.exits.north}: ${currentRoom.exits.north.direction}`)
        }
        if (currentRoom.exits.south) {
            console.log('South:', currentRoom.exits.south);
        }
        if (currentRoom.exits.east) {
            console.log('East:', currentRoom.exits.east);
        }
        if (currentRoom.exits.west) {
            console.log('West:', currentRoom.exits.west);
        }
    } else {
